  ...
  (async () => {
    const changeLinePrice = ()=>{
      if (data.coupon?.deduct) {
        ...
        data.carts[changePrice].product.price = (data.carts[changePrice].total - data.coupon.deduct) / data.carts[changePrice].qty
      }
...
try{
  const getOrder = await axios.post(`http://localhost:2407/createOrder/${id}`,
  {

...
console.log('cart', cart)
    ...
    //從首頁近來變成27個  但直接抓是30個   27個內不含該id
    useEffect(() => {
        const itemProduct = allProducts.find((val) => val.id === id);
        console.log(allProducts.map((val)=>val.id))
        console.log(allProducts,itemProduct, id)
        const otherProductList = filterOther(allProducts.filter((val) => { return val.category === itemProduct.category && val.id !== id }));
        setProduct(itemProduct);
        setOtherProducts(otherProductList);
        if (!itemProduct) return;
        setQuantity([itemProduct, ...otherProductList].map(val => { return { product_id: val.id, qty: 1 } }));
    }, [id, allProducts])

    const submit = async (id) => {
        const itemPrice = quantity.find((val) => { return val.product_id === id }).qty * allProducts.find((val) => { return val.id === id }).price;
        const itemQty = quantity.find((val) => { return val.product_id === id }).qty;
        if(!user.user){
            ...
            try {
                if (cart.length === 0) {
                    ...
                    await setDoc(doc(db, "carts", user.user.uid), {
                        carts: [{
                            'product': allProducts.find((val) => { return val.id === id }),
                            'qty': itemQty,
                            'total': itemPrice
                        }
                        ],
                    ...
                    const hadProduct = cartDoc.data().carts.findIndex(item => item.product.id === id);
    ...
    return (<>
        {product && <><div className="container-fluid bg-secondary px-0 mt-2">
                ...
                {otherProducts.map(product => {
                    return <div key={product.id} className="productsList myHover card text-center m-3 p-1" style={{ maxWidth: "300px" }}>
                        ...
                        <div className="card-body">
                            <Link to={`/product/${product.id}`} className="productsList">
                            ...
                            <button type="button" className="btn btn-dark mt-2" onClick={() => { submit(product.id) }}>
                                加入購物車
                            </button>
                        </div>
                    </div>
